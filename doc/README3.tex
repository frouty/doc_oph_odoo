...
\section{Installation de aeroolib}
...
cd laouestmonprojet
git add .
git commit -m 'un message d'info'
...
\begin{verbatim}
git diftool -t meld branch1 branch2
\end{verbatim}
...
On efface en général les branches qui sont merged
...
\begin{verbatim}
git checkout nom-de-la-branch-si-elle-existe
git checkout -b nom.de.la.branch.si.elle.existe.pas
\end{verbatim}
...
\subsection{Qd il y a des problemes au lancement d'odoo}
C'est parfois parce que la database qui est utilisée ne va pas donc
on peut filtrer le nom des database avec dans les arguments:
--db-filter [expression]

\subsection{comment supprimer une database}
...
\section{Chercher une string dans les fichiers d'un directory}
...
./opt/odoo/odoo7-server/openerp-server -c /etc/odoo/odoo7-server.conf
J'ai eu un probleme d'adresse deja utilisée. Dans odoo7-server.conf j'ai changé le wml_rpc port
il faut faire un kill -9 PID que l'on trouve avec ps aux | grep openerp et aussi ps aux | grep odoo.
...
\section{functional fields}
'field_name':fields.functional(_function_name, arg=None, fnct_inv=None,
fnct_inv_arg=None, type='a field type', fcn_search=None, obj=None,
method=False, store=False, multi=False)
\begin{itemize}
  \item fnct is the function or method that will compute the field value. It
  must have been declared before declaring the functional field.
  \item type is the field type name returned by the function. It can be any
  field type name except function.
  \item method = True si method dans l'objet et False si method globale.
\end{itemize}
def _function_name(self, cr, uid, ids, field_name, args, contex=None):
Je vois que quand je suis dans la fonction : _function_name
on a:
 field_name=local_day
args=None
ids = [4965L] je ne vois pas ce que c'est 
context ={'lang': 'fr_FR', 'tz': 'Pacific/Noumea', 'uid': 3, 'active_model':
'crm.meeting', 'calendar_default_user_id': 3, 'default_meeting_id': 16848, 'search_disable_custom_filters': True, 'search_default_meeting_id': 16848, 'active_ids': [16848], 'active_id': 16848}
et les active_id , active_ids cela doit etre l'id du report. 
...
\begin{itemize}
  \item 'lang' -> 'fr_FR'
  \item 'tz' -> 'Pacific/Noumea'
  \item 'uid' -> 3
  \item 'active_model -> 'oph.reporting'
  \item 'active_id' -> 295
  \item 'active_ids' -> [295]
  \item others 
\end{itemize}

\section{Comment obtenir un cursor sur une table}
mypool = self.pool.get('le nom de la table == nom du model')
ou
mypool = self.pool.get(context.get('active_model'))
\section{Comment obtenir des enregistrement de notre model}
mybrowse=mypool.browse(self,cr,self.uid,context.get('active_ids'))
si context.get('active_ids') est une liste avec un seul élement un seul numéro
d'enregistrement on obtient:
[browse_record(oph.reporting, 295)]
si context.get('active_ids') est une liste a plusieurs enregistrement on obtient
myids=[295.296]
mybrowses=mypool.browse(self.cr,self.uid,myids)
mybrowses
[browse_record(oph.reporting, 295), browse_record(oph.reporting, 296)]

\section{Comment le type d'un field deja existant}
Ce n'est pas vraiment possible. C'est un probleme pgsql.
Il n'est généralement pas possible de changer le type d'une colonne dans
postgresql meme si la colonne est vide. Sauf dans quelques cas.
\begin{itemize}
  \item integer to char
  \item donc field.Date, field.integer. field.many2one etc to a field.Char c'est
  OK. Et on peut donner le même nom. 
  \item field.char to field.Date pas possible, 
\end{itemize}
Si c'est pas possible il faut nommer le nouveau champ avec un autre nom.
...
\subsection{impression de la bonne date du jour sur les ordonnance par example}
le contexte: la date n'est pas la bonne sur les ordonnances de lunettes le matin
et devient correcte l'apres midi.
On a dans les odt:
\begin{itemize}
\item <date_report and formatLang(date_report,date=True) or ''> qui ne va pas
\item <o.date> qui ne va pas 
\item <formatLang(o.datewotime,date=True)> qui ne va pas 
\item <o.date and formatLang(o.date,date=True) or ''> qui ne va pas.
\end{itemize}

date_report est défini dans parser.py. mais je ne comprends pas j'ai
l'impression que l'on ne passe jamais dans la méthode 

Je vois que le champ date dans la vue crm.meeting est toujours bon aussi bien
pour la date que pour l'heure. 
Mais par contre <o.date> dans un odt ne marche pas. Et je ne vois que c'est une
erreur de tz.

Je vois que l'odt pour le memo de rdv  à la méthode only_time() qui marche.

\subsection{arrow}
import arrow
arrow.now().format('MM:DD/YYYY HH:mm') donne la date en local.

Si je veux savoir qu'elle est  la date en UTC par example: 
import pytz
arrow.now().to(pytz.UTC)

Si je fais arrow.now().replace(pytz.UTC) je ne change pas la date mais seulement
le TZ. Je ne vois pas quand cela peu servir.

\subsection{methode meeting_date}
def get_meeting_date(self, context=None):
 ...
 avec cela on a : rec.date qui donne une date mais ce n'est pas la bonne et je
 ne connais pas son tz elle est unaware.
 par exemple pour un rdv le 02.03.2018 a 8h cela donne rec.date='2018-03-01
 21:00:00'
 
 et
  arw = arrow.get(rec.date, 'YYYY-MM-DD HH:mm:ss')
  arw.to('local') donne le arrow à la bonne heure locale.
 mais ce n'est plus vrai sur le serveur car le serveur sont local est à UTC: 
 import time 
 time.tzname --> UTC
 contrairement dans un python shell qui donne +11
 donc 
 arw.to('Pacific/Noume') c'est bon mais je n'ai pas trouvé comme on fait pour
 trouver 'Pacific/Noumea'  et donc le changer dynamiquement.
...
\section{Comment mettre un champ dans un fichier ODT?}
libreoffice version 6.1.5.2. 
Insert / Field / More Fields / Functions / input field 
Est ce que cela marche avec cette nouvelle version de libreoffice à vérifier
...
\section{Changement de réseau}
Si on change de réseau avec changement d'adresse IP il faut:
\begin{itemize}
\item changer l'adresse iP du serveur dans /etc/network/ifconfig.
\item changer les adresses IP dans le fichier /etc/nginx/site-availabel/openerp
\item service nginx start et service nginx stop
\item mettre la regle de nat dans le routeur : network/firewall. 
\item ne pas oublier l'adresse IP du raspberry dans le raspberry mais aussi dans le fichier rt5100 (IP_client)
\end{itemize}
